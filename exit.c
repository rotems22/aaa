 ...
 */
static int will_become_orphaned_pgrp(int pgrp,
	...
	for_each_task(p)
	{
		if ((p == ignored_task) || (p->pgrp != pgrp)
				|| (p->state == TASK_ZOMBIE) || (p->p_pptr->pid == 1))
			continue;
...
static void exit_notify(void) {
	...
	if ((t->pgrp != current->pgrp) && (t->session == current->session)
...
NORET_TYPE void do_exit(long code) {
	...
	if (tsk->pid > 0) {
		...
		parent = tsk->p_pptr;
		pid_t parent_pid;
		parent_pid = parent->getpid();
		if (parent_pid > 0) {
			if (parent->max_zombies != 0) {
				list_add_tail(&(tsk->zombies_list), &(parent->zombies_list));
				parent->zombies_counter++;
			}
		}	
	}
	//CODE OF OURS
...
asmlinkage long sys_wait4(pid_t pid, unsigned int * stat_addr, int options,
	...
	do {
		...
		for (p = tsk->p_cptr; p; p = p->p_osptr) {
			...
			case TASK_STOPPED:
				...
				////////////////////////////////
			case TASK_ZOMBIE:
				if (tsk->max_zombies >= 0) {
					bool found = false;
					list_t* curNode;
					list_for_each(curNode, &(tsk->zombies_list))
					{
						if (curNode == &(p->zombies_list)) {
							found = true;
						}
					}
					if (found == true) {
	...
	retval = -ECHILD;
	end_wait4: current->state = TASK_RUNNING;
